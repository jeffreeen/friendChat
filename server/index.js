...
const server = http.createServer(async (req, res) => {
  try {
    if (/\/photos\/.+\.png/.test(req.url)) {
      ...
      const fallBackPath = path.resolve(__dirname, '../image/no-photo.png');
      const filePath = path.resolve(__dirname, '../image/photoUser', imageName);
      ...
      const filePath = path.resolve(__dirname, '../image/photoUser', `${name}.png`);
